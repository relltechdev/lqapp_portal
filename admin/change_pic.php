...
function changeProfilePic() {
	...
	if(strlen($name)) {
		...
		if(in_array($ext,$valid_formats)) {
			if($size<(1024*1024)) {
				$actual_image_name = 'avatar' .'_'.$userId.$iname.'.'.$ext;
...
function saveProfilePic(){
   ...
   $imagename=mysqli_escape_string($dbConn,$post['image_name']);
	



	$query="UPDATE lq_user SET image='$imagename' WHERE user_id='$userId'";
